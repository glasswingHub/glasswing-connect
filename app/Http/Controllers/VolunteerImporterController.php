...
class VolunteerImporterController extends Controller
{
    /**
    ...
    public function create()
    {
        ...
        $targetTableColumns = [
            'volunteerings' => $this->getTargetTableColumns('volunteerings'),
            'referenciasPersonalesVoluntarios' => $this->getTargetTableColumns('referenciasPersonalesVoluntarios'),
            'commitment_voluntary' => $this->getTargetTableColumns('commitment_voluntary'),
        ];

        $columnMappings = [
            'volunteerings' => [],
            'referenciasPersonalesVoluntarios' => [],
            'commitment_voluntary' => [],
        ];
        
        return Inertia::render('VolunteerImporters/Create', [
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'column_mappings.*.target_table' => 'required_with:column_mappings.*.target_table|string',
        ...
        if ($request->has('column_mappings') && is_array($request->column_mappings)) {
            foreach ($request->column_mappings as $table_name => $mappings) {
    ...
    public function show(string $id)
    {
        ...
        $columnMappings = [
            'volunteerings' => $this->getColumnMappings($importer, 'volunteerings'),
            'referenciasPersonalesVoluntarios' => $this->getColumnMappings($importer, 'referenciasPersonalesVoluntarios'),
            'commitment_voluntary' => $this->getColumnMappings($importer, 'commitment_voluntary'),
        ];
        ...
        return Inertia::render('VolunteerImporters/Show', [
            'importer' => $importer,
            'columnMappings' => $columnMappings,
    ...
    public function edit(string $id)
    {
        ...
        return Inertia::render('VolunteerImporters/Edit', [
            'importer' => $importer,
            'columnMappings' => $columnMappings,
            'sourceTableOptions' => $this->getSourceTableOptions(),
            'countries' => $this->getCountries(),
    ...
    public function getSourceTableColumns(Request $request)
    {
        ...
        try {
            $columns = collect(DB::connection('gwforms')
                ->select("SELECT COLUMN_NAME as name, DATA_TYPE as type FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ? ORDER BY COLUMN_NAME", [$request->table_name]))
                ->map(function ($column) {
                    return [
                        'value' => $column->name,
    ...
    private function getTargetTableColumns(string $table_name)
    {
        $columns = [];
