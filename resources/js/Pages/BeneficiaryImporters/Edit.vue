...
const fetchSourceTableColumns = async (tableName) => {
    ...
    try {
        const response = await window.fetchWithCSRF(route('beneficiary_importers.get-source-table-columns'), {
            method: 'POST',
...
const submit = () => {
    form.put(route('beneficiary_importers.update', props.importer.id));
};

const pageActions = [
    {
        href: route('beneficiary_importers.show', props.importer.id),
        label: 'Ver detalle'
    },
    {
    ...
    <AuthenticatedLayout>
        ...
        <form @submit.prevent="submit">
            ...
            <div v-if="form.target_table" class="mt-6">
                ...
                <div v-if="targetTableColumns.length > 0" class="bg-gray-50 rounded-lg p-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                        <div 
